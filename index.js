...
app.put("/listings/:title",async(req,res)=>{
    ...
    const params = [ntitle, ndescription, nprice, nlocation, ncountry, title];
    try{
        connection.query(`UPDATE info SET title = ?,description = ? ,price = ? ,location = ? ,country = ? WHERE title = ?`,params,(err,result)=>{
            if (err) throw err;
